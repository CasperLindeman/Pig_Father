...
class Player:
    ...
    def draw_cards(self, ncards, deck):
        ...
        for i, card in enumerate(cards_drawn):
            card.set_coordinate(100 + int(i * card.width * 11 / 10), 500)
...
############
    ...
    def blit_hand(self, x, y, screen):
        """
        OBS: x and y coordinates are not used.
        """
        for card in self.hand:
            card.blit(screen)
    ...
    def blit_board(self, x, y, screen):
        ...
        pygame.draw.rect(screen, (0,0,0), pygame.Rect(x-10,y-10, 290,375), 2)#Needs to be changed, use general numbers
        for j, row in enumerate(self.board):
            for i, card in enumerate(row):
                if j < 2:
                    card.set_coordinate(x + int(i*card.width*11/10), y + int((2-j)*card.height*5/4))
                else:
                    card.set_coordinate(x + int((i+1)*card.width*11/10), y + int((2-j)*card.height*5/4))
                card.blit(screen)
